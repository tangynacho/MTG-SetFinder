 ...
 */
export function pluginFactory(readFileFn, fileExistsFn) {
    ...
    return (config) => {
        ...
        let configFn = mifeConfig;
        let htmlXformFn = () => { return; };
        /**
         ...
         */
        function buildPeerModulePath(fileName) {
        ...
        async function buildExModule() {
            return (await readFile(buildPeerModulePath('vite-env.js'), { encoding: 'utf8' }))
         ...
         */
        async function loadImportMaps(command) {
            const cfg = config;
            ...
            if (fileCfg === undefined || typeof fileCfg === 'string') {
                ...
                for (let f of fileCfg) {
                    const contents = await readFile(f, { encoding: 'utf8' });
         ...
         */
        async function mifeConfig(viteOpts) {
        ...
        return {
            ...
            async config(cfg, opts) {
                viteEnv = opts;
                cssModuleFileName = viteEnv.command !== 'build' || config.cssStrategy === 'none' ?
                    'no-css.js' :
                    `${config.cssStrategy ?? 'singleMife'}-css.js`;
                ...
                async handler(_code, chunk, options, meta) {
                    ...
                    try {
                        ...
                        if (chunk.isEntry && !isRootConfig(config) && config.cssStrategy !== 'none') {
                            ...
                            const collectCssFiles = (curChunk) => {
                                if (!curChunk) {
                                    return;
                                }
